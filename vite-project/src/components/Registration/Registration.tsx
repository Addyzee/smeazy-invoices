...
import { Eye, EyeOff, Phone, Building2, User, Lock, ArrowRight, CheckCircle } from 'lucide-react'

// Main Registration Form Component
const RegistrationForm = () => {
  const [selectedRole, setSelectedRole] = React.useState<'business' | 'customer' | null>(null)
...
const RoleSelector: React.FC<RoleSelectorProps> = ({ onRoleSelect }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-primary/10 flex items-center justify-center p-4 sm:p-6">
      <div className="text-center w-full max-w-4xl mx-auto">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-3 sm:mb-4">Join Our Platform</h1>
        <p className="text-gray-600 mb-8 sm:mb-12 text-base sm:text-lg px-4">Choose how you'd like to get started</p>
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 justify-center items-center">
          <RoleCard
            ...
            description="Create your business profile and reach customers"
            icon={Building2}
...
const RoleCard: React.FC<RoleCardProps> = ({ role, title, subtitle, description, icon: Icon, onClick }) => {
  const gradients = {
    business: 'from-secondary to-accent',
    customer: 'from-primary to-secondary'
  }
  ...
  return (
    ...
    >
      <div className="w-full h-72 sm:h-80 bg-white rounded-2xl shadow-xl border border-gray-100 group-hover:shadow-2xl transition-all duration-300 overflow-hidden">
        <div className={`h-20 sm:h-24 bg-gradient-to-r ${gradients[role]} relative`}>
        ...
        <div className="p-4 sm:p-6">
          ...
          <p className={`text-sm font-medium mb-3 bg-gradient-to-r ${gradients[role]} bg-clip-text text-transparent`}>
            {subtitle}
          </p>
          <p className="text-gray-600 text-sm mb-4 sm:mb-6 leading-relaxed">{description}</p>
          
          <div className={`w-full py-2.5 sm:py-2 px-4 rounded-lg bg-gradient-to-r ${gradients[role]} text-white font-medium text-center transition-all duration-300 flex items-center justify-center gap-2 text-sm sm:text-base`}>
...
const FormCard: React.FC<FormCardProps> = ({ role, formData, onInputChange, onSubmit, onBack }) => {
  const gradients = {
    business: 'from-secondary to-accent',
    customer: 'from-primary to-secondary'
  }
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
        {/* Header */}
        <div className={`h-32 bg-gradient-to-r ${gradients[role]} relative`}>
          <div className="absolute inset-0 bg-black/10"></div>
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{config[role].title}</h2>
            <p className="text-gray-600">{config[role].subtitle}</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                type="submit"
                className={`flex-1 py-3 px-4 rounded-xl bg-gradient-to-r ${gradients[role]} text-white font-semibold hover:shadow-lg transform hover:scale-[1.02] transition-all duration-200 flex items-center justify-center gap-2`}
              >
...
const InputField: React.FC<InputFieldProps> = ({ label, type, value, onChange, placeholder, icon: Icon, required }) => {
  return (
...
const PasswordField: React.FC<PasswordFieldProps> = ({ label, value, onChange, placeholder, required }) => {
