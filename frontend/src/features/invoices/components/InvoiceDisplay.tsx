  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6 hover:shadow-xl hover:bg-white/90 transition-all duration-300">
      <div className="flex items-center gap-3 mb-6">
        <div className={`w-10 h-10 bg-gradient-to-r ${theme?.primary || 'from-primary to-secondary'} rounded-xl flex items-center justify-center shadow-lg`}>
...
const InfoRow: React.FC<{
  ...
  return (
    <div className={`${className} flex items-center justify-between py-4 border-b border-gray-100/50 last:border-b-0 group hover:bg-gray-50/50 rounded-lg px-2 -mx-2 transition-all duration-200`}>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <div className={`w-8 h-8 bg-gradient-to-r ${theme?.primary || 'from-primary to-secondary'} text-white rounded-xl flex items-center justify-center text-xs font-bold shadow-lg`}>
              {index + 1}
            </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="text-center">
          <div className="bg-gradient-to-r from-gray-100 to-gray-200 rounded-xl p-3 mb-2">
            ...
            <p className="font-bold text-gray-900 text-lg">KES {item.unit_price.toFixed(2)}</p>
          </div>
        </div>
        <div className="text-center">
          <div className={`bg-gradient-to-r ${theme?.accent || 'from-primary/10 to-secondary/10'} rounded-xl p-3 mb-2 border ${theme?.border || 'border-primary/20'}`}>
            <p className="text-xs font-medium text-gray-600 uppercase tracking-wide mb-1">Total</p>
            <p className={`font-bold text-lg ${theme?.text || 'text-gray-900'}`}>
...
export const InvoiceDisplay: React.FC<InvoiceDisplayProps> = ({
  ...
  const invoice = useInvoiceStore((state) => state.currentInvoice) as unknown as InvoiceType ;
  ...
  const onEdit = () => setPopUpType("update");
  const onDelete = () => {
    ...
    <div className="invoice-scroll-container h-full w-full overflow-y-auto">
      ...
      <div className={`min-h-full bg-gradient-to-br ${theme.accent} relative`}>
        ...
        <div className="absolute inset-0 opacity-30">
          <div className={`absolute top-20 left-20 w-64 h-64 bg-gradient-to-r ${theme.primary} rounded-full blur-3xl opacity-20`}></div>
          <div className={`absolute bottom-20 right-20 w-96 h-96 bg-gradient-to-r ${theme.secondary} rounded-full blur-3xl opacity-15`}></div>
        </div>
        ...
        <div className="relative z-10 py-8 px-6">
          <div className="max-w-3xl mx-auto">
            ...
            <div className={`sticky top-0 z-20 transition-all duration-300 mb-8 ${
              isScrolled 
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-4 mb-3">
                    <div className={`w-16 h-16 bg-gradient-to-r ${theme.primary} rounded-2xl flex items-center justify-center shadow-xl`}>
                    ...
                    <div>
                      ...
                      <p className="text-lg text-gray-600 font-medium">{invoice.business_name}</p>
                    </div>
                  </div>
                </div>
                
                <div className="text-right">
                  <div className={`bg-gradient-to-r ${theme.accent} rounded-2xl p-4 border ${theme.border} shadow-lg`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              <ViewSection title="Business Information" icon={Building2} status={invoice.status}>
                <InfoRow
                  label="Business Name"
              ...
              <ViewSection title="Customer Information" icon={User} status={invoice.status}>
                <InfoRow
                  label="Full Name"
            ...
            {/* Invoice Details Row */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <ViewSection title="Invoice Details" icon={Calendar} status={invoice.status}>
                <InfoRow
                  label="Invoice #"
              ...
              {/* Summary Section */}
              <ViewSection title="Summary" icon={Calculator} status={invoice.status}>
                <div className={`bg-gradient-to-r ${theme.accent} rounded-xl p-5 border ${theme.border} shadow-inner`}>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className={`bg-gradient-to-r ${theme.secondary} rounded-lg px-3 py-1`}>
                      <span className={`font-bold ${theme.text}`}>{invoice.line_items.length}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-gray-700">Average Item Value</span>
                    <span className={`font-bold ${theme.text}`}>
                      KES {(invoice.total_amount / invoice.line_items.length).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                    </span>
                  </div>
                </div>
              </ViewSection>
            ...
            {/* Line Items */}
            <ViewSection title="Invoice Items" icon={Package} status={invoice.status}>
              <div className="space-y-6">
                ...
                {/*  Total Summary */}
                <div className={`bg-gradient-to-r ${theme.accent} rounded-2xl p-8 border-2 ${theme.border} shadow-xl mt-8 relative overflow-hidden`}>
                  {/* Decorative background element */}
                  <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-r ${theme.primary} rounded-full blur-2xl opacity-20 transform translate-x-16 -translate-y-16`}></div>
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-600 mb-1">Grand Total</p>
                          <p className={`text-3xl font-bold ${theme.text}`}>
                            KES {invoice.total_amount.toLocaleString()}
                          </p>
                        </div>
                      ...
                      <div className="text-right">
                        <div className={`bg-gradient-to-r ${theme.secondary} rounded-xl p-3`}>
            ...
            {invoice.notes && (
              <div className="mt-8">
                <ViewSection title="Additional Notes" icon={FileText} status={invoice.status}>
                  <div className={`bg-gradient-to-r ${theme.accent} rounded-xl p-6 border ${theme.border} shadow-inner`}>
                    <div className="flex items-start gap-3">
                      <div className={`w-8 h-8 bg-gradient-to-r ${theme.secondary} rounded-lg flex items-center justify-center flex-shrink-0 mt-1`}>
                        <FileText className={`w-4 h-4 ${theme.text}`} />
                      </div>
                      <p className="text-gray-800 leading-relaxed font-medium">{invoice.notes}</p>
                    </div>
                  </div>
                </ViewSection>
              </div>
